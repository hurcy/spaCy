...
def test_ko_tokenizer_handles_basic_contraction(en_tokenizer):
    text = "don't giggle"
...
def test_ko_tokenizer_handles_basic_contraction_punct(en_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 3
...
def test_ko_tokenizer_handles_poss_contraction(en_tokenizer, text_poss, text):
...
def test_ko_tokenizer_splits_trailing_apos(en_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 2
...
def test_ko_tokenizer_doesnt_split_apos_exc(en_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 1
    assert tokens[0].text == text
...
def test_ko_tokenizer_handles_ll_contraction(en_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 2
    assert tokens[0].text == text.split("'")[0]
...
def test_ko_tokenizer_handles_capitalization(en_tokenizer, text_lower, text_title):
...
def test_ko_tokenizer_keeps_title_case(en_tokenizer, pron, contraction):
...
def test_ko_tokenizer_excludes_ambiguous(en_tokenizer, exc):
...
def test_ko_tokenizer_splits_defined_punct(en_tokenizer, wo_punct, w_punct):
...
def test_ko_tokenizer_handles_abbr(en_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 1


def test_ko_tokenizer_handles_exc_in_text(en_tokenizer):
...
def test_ko_tokenizer_handles_times(en_tokenizer, text):
    tokens = en_tokenizer(text)
    assert len(tokens) == 2
...
def test_ko_tokenizer_norm_exceptions(en_tokenizer, text, norms):
    tokens = en_tokenizer(text)
...
def test_ko_lex_attrs_norm_exceptions(en_tokenizer, text, norm):
    tokens = en_tokenizer(text)
    assert tokens[0].norm_ == norm


@pytest.mark.parametrize("text", ["faster", "fastest", "better", "best"])
def test_ko_lemmatizer_handles_irreg_adverbs(en_tokenizer, text):
    tokens = en_tokenizer(text)
